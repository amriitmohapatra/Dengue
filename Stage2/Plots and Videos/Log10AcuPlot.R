...
panel.plotter <- function(data_type, true_matrix, predicted_matrix, ticks, png_lb, png_ub, week_num, letter_counter, r){
  ...
  if(week_num == 4 || week_num == 8 || week_num == 12){
  ...
  if(data_type == 1){
    ...
    # grid.text(paste("Week", weeks, " Training Set",sep = " "), x = unit(4.2, 'lines'), y = 0.95, gp = gpar(fontsize = 7))
  }else if(data_type == 2){
    if(weeks > 9){
      grid.text(paste(letters[letter_counter],sep = ""), x = unit(1, 'lines'), y = 0.95, gp = gpar(fontsize = 10, fontface = "bold"))
      grid.text(paste("Week ", weeks, " Out of Sample",sep = ""), x = unit(7.7, 'lines'), y = 0.94, gp = gpar(fontsize = 8))
      # grid.text(paste("(",letters[letter_counter],")", " Week ", weeks, " Out of Sample",sep = ""), x = unit(5.25, 'lines'), y = 0.95, gp = gpar(fontsize = 8))
    }else{
      grid.text(paste(letters[letter_counter],sep = ""), x = unit(1.3, 'lines'), y = 0.95, gp = gpar(fontsize = 10, fontface = "bold"))
      grid.text(paste("Week ", weeks, " Out of Sample",sep = ""), x = unit(7.7, 'lines'), y = 0.94, gp = gpar(fontsize = 8))
...
########################### Auto Run ################################
letter_counter = 1
filename <- paste("LogAcuPlot1" ,".png", sep = "")
...
for(weeks in 1:4){
  for(i in data_type){
    ...
    panel.plotter(data_type = data_type_num, true_matrix = op_set$true_matrix, predicted_matrix = op_set$predicted_matrix, ticks = beautifier$ticks, png_lb = beautifier$png_lb, png_ub = beautifier$png_ub, week_num = weeks, letter_counter = letter_counter, r = op_set$r)
...
filename <- paste("LogAcuPlot2" ,".png", sep = "")
png(file = filename,height=24,width=15,units='cm',res=300,pointsize=10)
pushViewport(plotViewport(c(2,2,1,1)))
pushViewport(viewport(layout=grid.layout(nrow=4,ncol=2)))
for(weeks in 5:8){
...
letter_counter = letter_counter
filename <- paste("LogAcuPlot3" ,".png", sep = "")
png(file = filename,height=24,width=15,units='cm',res=300,pointsize=10)
pushViewport(plotViewport(c(2,2,1,1)))
pushViewport(viewport(layout=grid.layout(nrow=4,ncol=2)))
for(weeks in 9:12){
