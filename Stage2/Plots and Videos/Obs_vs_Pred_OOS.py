...
import sys
os.getcwd()
folder_path = "C:\\Users\\amritm\\nBox\\May-Jun 2024\\Final Code\\Stage2\\Plots and Videos"
...
def plot_week(week_number, vmin, vmax, prediction_data_df, true_case_data_df, output_prefix, sg_regions_gdf):
    ...
    #cbar.set_ticks(np.linspace(0, vmax, num=5))  # Adjust the number of ticks as needed
...
for i in range(1, 13):
    prediction_data_path = folder_path + f'\\W_{i}_Predictions_OutofSample.csv'
    true_case_data_path = folder_path + f'\\W_{i}_True_OutofSample.csv'
    ...
    for week in week_numbers:
        image_path = plot_week(week, global_min_cases, global_max_cases, prediction_data_df, true_case_data_df, f'out_{i}', sg_regions_gdf)
        image_paths.append(image_path)

    output_video_path = folder_path + f'\\fix_out_{i}.mp4'
