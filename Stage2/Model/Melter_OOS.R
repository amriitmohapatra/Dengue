...
for(weeks in 1:length(week_looper)){
  ...
  setwd(dir_path_2)
  write.csv(df_dev, file = paste("W_", weeks, "_Deviation_OutofSample.csv",sep = ""), row.names = FALSE)
  write.csv(df_true, file = paste("W_", weeks, "_True_OutofSample.csv",sep = ""), row.names = FALSE)
  write.csv(df_pred, file = paste("W_", weeks, "_Predictions_OutofSample.csv",sep = ""), row.names = FALSE)
  setwd(dir_path_3)
  write.csv(df_dev, file = paste("W_", weeks, "_Deviation_OutofSample.csv",sep = ""), row.names = FALSE)
  write.csv(df_true, file = paste("W_", weeks, "_True_OutofSample.csv",sep = ""), row.names = FALSE)
  write.csv(df_pred, file = paste("W_", weeks, "_Predictions_OutofSample.csv",sep = ""), row.names = FALSE)
