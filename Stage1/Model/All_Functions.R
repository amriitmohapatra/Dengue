LASSO_Model <- function(YYY){
  set.seed(0)
  X_train = X_test = YYY[,-ncol(YYY)]
...
SVR1_Model <- function(YYY){
  training_set = test_set = YYY
  set.seed(0)
  # Feature Scaling
...
LSTM_Model <- function(YYY){
  ...
  regressor = keras_model_sequential() %>%
    layer_lstm(units = 200, return_sequences = TRUE, input_shape = c(dim(X_train)[2], dim(X_train)[3]))%>%
  ...
  # regressor %>% save_model_hdf5(paste("C:/Users/amritm/nBox/Sep-Oct/Dengue_5_Regions_NewExperiment/Two Stage Regression/LSTM_","Model_",models,".hdf5", sep = ""))
  
  regressor %>% save_model_hdf5(paste(dir_path,"/Model/LSTM_","Model_",models,".hdf5", sep = ""))
  inputs = test_set
...
GRU_Model <- function(YYY){
  ...
  # Building the LSTM RNN
  regressor = keras_model_sequential() %>%
    layer_gru(units = 200, return_sequences = TRUE, input_shape = c(dim(X_train)[2], dim(X_train)[3]))%>%
    layer_dropout(rate = 0.15) %>%
    layer_gru(units = 150, return_sequences = TRUE)%>%
    layer_dropout(rate = 0.15)%>%
    layer_gru(units = 100, return_sequences = TRUE)%>%
    layer_dropout(rate = 0.15)%>%
    layer_gru(units = 50, return_sequences = TRUE)%>%
    layer_dropout(rate = 0.15)%>%
    layer_gru(units = 25, return_sequences = FALSE)%>%
    layer_dropout(rate = 0.15)%>%
    layer_dense(units = 1)
  ...
  # regressor %>% save_model_hdf5(paste("C:/Users/amritm/nBox/Sep-Oct/Dengue_5_Regions_NewExperiment/Two Stage Regression/GRU_","Model_",models,".hdf5", sep = ""))
  
  regressor %>% save_model_hdf5(paste(dir_path,"/Model/GRU_","Model_",models,".hdf5", sep = ""))
  inputs = test_set
